...
\begin{enumerate}[(a)]
	\item  Let $\mathcal{H} = \{H_s : \bits^{2n} \to \bits^n\}_{s\in\bits^\lambda}$ be a family of collision-resistant hash
	      ...
	      \begin{enumerate}[(i)]
		      ...
		      \item What happens in case the set of functions $\mathcal{H}$ is not compressing (i.e., the domain of each function $H_s$ is also $\bits^n$? Does collision-resistance imply one-way in this case?
